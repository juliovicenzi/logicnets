...
class UnswNb15NeqModel(nn.Module):
    def __init__(self, model_config):
        ...
        self.num_neurons = [model_config["input_length"]] + \
            model_config["hidden_layers"] + [model_config["output_length"]]
        layer_list = []
        for i in range(1, len(self.num_neurons)):
            ...
            if i == 1:
                ...
                mask = RandomFixedSparsityMask2D(
                    in_features, out_features, fan_in=model_config["input_fanin"])
                ...
                layer = SparseLinearNeq(in_features,
                                        out_features,
    ...
    def verilog_inference(self, verilog_dir, top_module_filename,
                          logfile: bool = False, add_registers: bool = False):
    ...
    def verilog_forward(self, x):
        ...
        for i in range(x.shape[0]):
            ...
            res_split = [result[i:i + output_bitwidth]
                         for i in range(0, len(result), output_bitwidth)][::-1]
    ...
    def forward(self, x):
        if self.is_verilog_inference:
            ...
            x = self.module_list[-1].output_quant.apply_post_transforms(
                (x - 2**(output_bits - 1)) * output_scale)
        else:
